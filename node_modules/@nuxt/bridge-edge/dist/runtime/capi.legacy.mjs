...
import { useRouter as _useRouter, useRoute as _useRoute, useState } from "./composables.mjs";
...
const unsupported = (message) => () => {
  throw new Error(`${msgPrefix} ${message} ${checkDocsMsg}`);
};
const _warned = {};
...
export const ssrRef = (value, key) => {
  const vm = getVM();
  if (!vm) {
    throw new Error("ssrRef no longer supports global/ambient context and must be called within a setup() function");
  }
...
export const shallowSsrRef = (value, key) => {
  ...
  const ref2 = ssrRef(value, key);
  if (process.client) {
    return shallowRef(ref2.value);
  }
  return ref2;
};
export const ssrPromise = (value, key) => {
  ...
  onServerPrefetch(async () => {
    ssrRefs[key] = sanitise(await promise);
  });
  return promise;
};
...
export const useStore = () => {
  ...
  return getCurrentInstance2().$store;
};
const fetches = /* @__PURE__ */ new WeakMap();
...
function getKey(vm) {
  ...
  const getCounter = createGetCounter(
    process.server ? vm.$ssrContext.fetchCounters : nuxt.vue2App._fetchCounters,
    defaultKey
  );
...
async function serverPrefetch(vm) {
  ...
  vm._fetchKey = // Nuxt 2.15+ uses a different format - an object rather than an array
  "push" in vm.$ssrContext.nuxt.fetch ? vm.$ssrContext.nuxt.fetch.length : vm._fetchKey || vm.$ssrContext.fetchCounters[""]++;
  if (!vm.$vnode.data) {
    vm.$vnode.data = {};
  }
  ...
  const data = Object.fromEntries(
    Object.entries(vm?._setupProxy || vm?._setupState).filter(
      ([_key, val]) => !(val && typeof val === "object" && "_compiled" in val || val instanceof Function || val instanceof Promise)
