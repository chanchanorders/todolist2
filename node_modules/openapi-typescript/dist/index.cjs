...
async function load(schema, options) {
  ...
  if (schema instanceof import_node_url.URL) {
    ...
    const ext = import_node_path2.default.extname(schema.pathname).toLowerCase();
    if (schema.protocol.startsWith("http")) {
      ...
      if (ext === ".json" || (contentType == null ? void 0 : contentType.includes("json"))) {
        options.schemas[schemaID] = {
          hint,
          schema: parseJSON(await res.text())
        };
      } else if (ext === ".yaml" || ext === ".yml" || (contentType == null ? void 0 : contentType.includes("yaml"))) {
        options.schemas[schemaID] = {
          hint,
  ...
  walk(currentSchema, (rawNode, nodePath) => {
    ...
    if (schema instanceof import_node_url.URL) {
      const nextURL = new import_node_url.URL(ref.filename, schema);
    ...
    if (isRemoteURL(ref.filename) || isFilepath(ref.filename)) {
      const nextURL = new import_node_url.URL(ref.filename.startsWith("//") ? `https://${ref.filename}` : ref.filename);
...
function defaultSchemaObjectTransform(schemaObject, { path: path2, ctx }) {
  ...
  if ("properties" in schemaObject && schemaObject.properties && Object.keys(schemaObject.properties).length || "additionalProperties" in schemaObject && schemaObject.additionalProperties) {
    indentLv++;
    for (const [k, v] of getEntries((_b = schemaObject.properties) != null ? _b : {}, ctx.alphabetize, ctx.excludeDeprecated)) {
...
${indent("}", indentLv)}` : "";
  function collectCompositions(items) {
    ...
    for (const item of items) {
      ...
      if ("$ref" in item && ((_a2 = ctx.discriminators[item.$ref]) == null ? void 0 : _a2.mapping)) {
...
async function openapiTS(schema, options = {}) {
  ...
  const ctx = {
    ...
    defaultNonNullable: (_c = options.defaultNonNullable) != null ? _c : false,
    discriminators: {},
    ...
    immutableTypes: (_d = options.immutableTypes) != null ? _d : false,
    emptyObjectsUnknown: (_e = options.emptyObjectsUnknown) != null ? _e : false,
    indentLv: 0,
    operations: {},
    pathParamsAsTypes: (_f = options.pathParamsAsTypes) != null ? _f : false,
    parameters: {},
    silent: (_g = options.silent) != null ? _g : false,
    supportArrayLength: (_h = options.supportArrayLength) != null ? _h : false,
  ...
  await load(schemaURL, {
    ...
    auth: options.auth,
    schemas: allSchemas,
    rootURL: isInlineSchema ? new import_node_url2.URL(VIRTUAL_JSON_URL) : schemaURL,
    // if an inline schema is passed, use virtual URL
    urlCache: /* @__PURE__ */ new Set(),
    httpHeaders: options.httpHeaders,
    httpMethod: options.httpMethod,
    fetch: (_j = options.fetch) != null ? _j : getDefaultFetch()
  });
        ...
        case "SchemaMap": {
          ...
          outer:
            for (const [name, schemaObject] of getEntries(subschema.schema)) {
              ...
              const c2 = getSchemaObjectComment(schemaObject, indentLv);
              if (c2)
                subschemaOutput += indent(c2, indentLv);
              if (!("type" in schemaObject) && !("$ref" in schemaObject)) {
                for (const method of ["get", "put", "post", "delete", "options", "head", "patch", "trace"]) {
                  if (method in schemaObject) {
                    subschemaOutput += indent(`${escObjKey(name)}: ${transformPathItemObject(schemaObject, { path: `${path2}${name}`, ctx: { ...ctx, indentLv } })};
`, indentLv);
                    continue outer;
                  }
                }
              }
              subschemaOutput += indent(`${escObjKey(name)}: ${transformSchemaObject(schemaObject, { path: `${path2}${name}`, ctx: { ...ctx, indentLv } })};
`, indentLv);
            }
          indentLv--;
          subschemaOutput += indent("};", indentLv);
